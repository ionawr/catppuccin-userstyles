...
if grep -q '@base: #1e1e2e' lib/std/v1.less; then
  echo "ERROR: Palette replacement failed in lib/std/v1.less (still has upstream mocha)"
  errors=$((errors + 1))
fi

# check filters were replaced (spot-check mocha filter for fork-specific value)
if ! grep -q '@base: brightness(0) saturate(100%) invert(0%) sepia(48%)' lib/std/v1.less; then
  echo "ERROR: Mocha filter replacement failed in lib/std/v1.less"
  errors=$((errors + 1))
fi
